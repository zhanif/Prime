module.exports.run = async(bot, message, args, db) =>{
        ...
        let setAutorole = {$set : {autorole:idrole}};
        db.collection('Data').findOneAndUpdate(serverID, setAutorole).catch(err =>{
            if(err) return console.log(err);
        });
        if(idrole!="OFF") return message.channel.send(`:gear: The Auto-role has been updated to **${check.name}**`);
        if(idrole == "OFF") return message.channel.send(`:gear: The Auto-role has been turned off`);
}
module.exports.help = {
	name:"autorole"
}