module.exports.run = async(bot, message, args, db) =>{
    ...
    let setChannel = {$set : {votechannel:idchn}};
    db.collection('Data').findOneAndUpdate(serverID, setChannel).catch(err =>{
        if(err) return console.log(err);
    });
    if(idchn!="OFF") return message.channel.send(`:gear: The Vote-channel has been updated to **#${check.name}**`);
    if(idchn == "OFF") return message.channel.send(`:gear: The Vote-channel has been turned off`);
}
module.exports.help = {
	name:"votechannel"
}